/* HOY 2.8 — private operator menu intake: upload, official URL, direct structured entry */
(function(){
  const MAX_BYTES=25*1024*1024;
  const MIME_OK=new Set(['application/pdf','image/jpeg','image/png','image/webp']);
  const intakeState={restaurantId:null,rows:[],loading:false};
  ...
  function statusText(v){return ({uploaded:'Sicher hochgeladen',queued:'Zur Strukturierung vorgemerkt',processing:'Wird strukturiert',review_required:'HOY-Prüfung nötig',operator_review:'Zur Betreiberprüfung bereit',approved:'Freigegeben',rejected:'Abgelehnt',published:'Veröffentlicht',failed:'Verarbeitung fehlgeschlagen'})[v]||v||'Neu'}
  function processorText(v){return ({not_started:'Noch nicht gestartet',queued:'In Warteschlange',extracting:'Strukturierung läuft',structured:'Strukturierter Entwurf',needs_review:'Prüfung erforderlich',failed:'Fehlgeschlagen'})[v]||v||''}
  ...
  async function loadRows(p,force=false){
    ...
    const {data,error}=await sb.from('menu_intake_submissions')
      .select('id,restaurant_id,source_kind,publication_mode,original_filename,source_url,status,processor_state,processor_note,submitted_at,processed_at,operator_confirmed_at,published_at')
  ...
  function cardHTML(p){
    ...
    return `<section class="menu-intake-owner-card ${paid?'paid':'free'}">
      ...
      <p>${paid?'Du kannst neue Karten selbst einreichen und als Live-Entwurf vorbereiten. Veröffentlichung erfolgt erst nach strukturierter Prüfung und Bestätigung.':'Du kannst deine Karte kostenlos zur HOY-Prüfung einreichen. Pro/Business ergänzt den laufenden Self-Service für aktuelle Karten.'}</p>
  ...
  const basePartner28=partner;
  partner=function(){
    const html=basePartner28();const p=currentRestaurant();if(!p)return html;
  ...
  function openDialog(p){
    const d=ensureDialog();const mode=publicationMode(p);const paid=paidPlan(p);
    d.innerHTML=`<div class="menu-intake-flow">
      ...
      <p class="claim-lead">Originalquelle sichern → strukturierten Entwurf erzeugen → prüfen → erst dann veröffentlichen. HOY übernimmt niemals ungeprüfte OCR-Ergebnisse direkt in die Gastkarte.</p>
      <div class="menu-intake-methods">
        <section><small>PDF / FOTO</small><h3>Datei hochladen</h3><p>PDF, JPG, PNG oder WebP · maximal 25 MB. Die Datei bleibt privat.</p><input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" data-menu-file><button data-menu-upload>Datei sicher einreichen</button></section>
  ...
  async function submitFile(p,d){
    ...
    try{
      ...
      await invokeProcessor(row.id);await loadRows(p,true);toast('Speisekarte sicher eingereicht');openDialog(p);
  ...
  async function submitUrl(p,d){
    ...
    try{
      ...
      if(error)throw error;await invokeProcessor(row.id);await loadRows(p,true);toast('Offizieller Menülink eingereicht');openDialog(p);
  ...
  const baseWire28=wire;
  wire=function(){
    baseWire28();
    const p=currentRestaurant();
    document.querySelectorAll('[data-menu-intake-open]').forEach(btn=>btn.onclick=()=>openDialog(p));
    if(state.view==='partner'&&p&&cloud.user&&intakeState.restaurantId!==Number(p.id))loadRows(p);
  };

  const baseAuthLogout28=authLogout;
  authLogout=async function(){intakeState.restaurantId=null;intakeState.rows=[];await baseAuthLogout28()};
})();
