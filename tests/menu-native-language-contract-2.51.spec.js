...
test('guest never sees preparation placeholders or source-only menu substitutes',async({page})=>{
  await ready(page);
...
test('menu is visible only as a complete HOY-native menu in the current page language',async({page})=>{
  ...
  await page.evaluate(id=>{
    state.lang='de';
    MENUS[id]={
      nativeMenu:true,integrity:'complete',guestAvailability:'in_app_native',localized:true,locale:'de',
  ...
  await page.evaluate(id=>{
    document.getElementById('detail')?.close();
    MENUS[id]={
      nativeMenu:true,integrity:'complete',guestAvailability:'in_app_native',localized:true,locale:'en',
...
test('strict helper rejects partial, wrong-language and external-only states',async({page})=>{
  ...
  const result=await page.evaluate(()=>({
    wrong:window.hoyGuestMenuReady251({nativeMenu:true,localized:true,guestAvailability:'in_app_native',locale:'es',categories:[['A',[['X','1 €']]]],languageCoverage:{locale:'es',total:1,ready:1,complete:true}},'de'),
    partial:window.hoyGuestMenuReady251({nativeMenu:true,localized:true,guestAvailability:'in_app_native',locale:'de',categories:[['A',[['X','1 €']]]],languageCoverage:{locale:'de',total:2,ready:1,complete:false}},'de'),
    external:window.hoyGuestMenuReady251({nativeMenu:false,localized:false,guestAvailability:'external_reference',officialMenuUrl:'https://example.com/menu.pdf'},'de'),
    good:window.hoyGuestMenuReady251({nativeMenu:true,localized:true,guestAvailability:'in_app_native',locale:'de',categories:[['A',[['X','1 €']]]],languageCoverage:{locale:'de',total:1,ready:1,complete:true}},'de')
  }));
