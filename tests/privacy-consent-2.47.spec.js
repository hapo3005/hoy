...
const CONSENT='hoy-privacy-analytics-consent-v1';
const PREF='hoy-privacy-preference-v1';
const STALE='hoy-analytics-consent-v1';

async function resetPrivacy(page){
  await page.goto('/?privacy_qa=1');
  await page.evaluate(({CONSENT,PREF,STALE})=>{
    [CONSENT,PREF,STALE,'hoy-anonymous-id-v1','hoy-proof-pilot-code-v1','hoy-proof-pilot-enrolled-v1','hoy-analytics-v1'].forEach(k=>localStorage.removeItem(k));
    sessionStorage.removeItem('hoy-session-id-v1');
  },{CONSENT,PREF,STALE});
  await page.reload();
}

test('privacy choice is explicit, canonical and accept/reject are co-located', async ({ page }) => {
  ...
  const state=await page.evaluate(({CONSENT,PREF,STALE})=>({
    consent:localStorage.getItem(CONSENT),
    stale:localStorage.getItem(STALE),
    pref:JSON.parse(localStorage.getItem(PREF))
  }),{CONSENT,PREF,STALE});
  expect(state.consent).toBe('granted');
  expect(state.stale).toBeNull();
  expect(state.pref.choice).toBe('granted');
  expect(state.pref.noticeVersion).toBe('rt008-draft-2026-08-19');
  expect(state.pref.decidedAt).toBeTruthy();
});
...
test('withdrawal is available from persistent privacy settings and clears analytics identifiers', async ({ page }) => {
  ...
  const values=await page.evaluate(()=>({
    consent:localStorage.getItem('hoy-privacy-analytics-consent-v1'),
    stale:localStorage.getItem('hoy-analytics-consent-v1'),
  ...
  expect(values).toEqual({consent:'rejected',stale:null,anon:null,pilot:null,sent:null,history:null,session:null});
});
