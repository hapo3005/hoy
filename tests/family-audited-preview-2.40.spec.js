...
async function expectedFamilyCount(page){
...
test('live verified Family data takes precedence over the audited research preview without losing the research inventory',async({page})=>{
  ...
  expect(initial.ready).toBeGreaterThanOrEqual(17);
  ...
  expect(initial.liveIds.length).toBeGreaterThan(0);
  expect(initial.locked).toBe(3);

  const expected=await expectedFamilyCount(page);
  ...
  await expect(page.locator('[data-journey-results] .list-card[data-open]')).toHaveCount(expected);
...
test('audited research inventory keeps unknown Family geometry unknown while remaining inert beside live data',async({page})=>{
  ...
  expect(facts.total).toBeGreaterThanOrEqual(17);
  expect(facts.unknownDistance).toBe(facts.total);
