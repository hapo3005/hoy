...
test('RT-007 first-party opportunity is screened, bounded and reconciled to current hard queue', async()=>{
  expect(summary.schemaVersion).toBe('1.0.1');
  ...
  expect(summary.factVerificationStillRequired).toBe(47);
  ...
  expect(summary.byField.location_source_url.preparedDryRunReferences).toBe(2);
...
test('RT-007 opportunity ceiling cannot masquerade as approved rights or completed replacements', async()=>{
  expect(summary.preparedCandidates.combinedAfterRequiredRebase.references).toBe(17);
  expect(summary.preparedCandidates.combinedAfterRequiredRebase.projectedHardQueue).toBe(312);
