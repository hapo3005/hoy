...
test('RT-007 first-party opportunity is screened, bounded and reconciled to current hard queue', async()=>{
  expect(summary.schemaVersion).toBe('1.3.0');
  ...
  expect(summary.factVerificationStillRequired).toBe(33);
...
test('RT-007 opportunity ceiling cannot masquerade as approved rights or completed replacements', async()=>{
  ...
  expect(summary.preparedCandidates.signatureWave2.pageContentRecheckBeforeApplyRequired).toBe(true);
  expect(summary.preparedCandidates.combinedAfterRequiredRebase.references).toBe(34);
  expect(summary.preparedCandidates.combinedAfterRequiredRebase.projectedHardQueue).toBe(295);
