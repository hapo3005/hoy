...
test('G1 Acquisition Clean master register is internally consistent',async()=>{
  const output=execFileSync(process.execPath,['scripts/check-acquisition-clean.cjs'],{cwd:process.cwd(),encoding:'utf8'});
  const result=JSON.parse(output);
  expect(result.ok).toBe(true);
  expect(result.gate).toBe('G1_ACQUISITION_CLEAN');
  expect(result.schemaVersion).toBe('1.1.1');
  ...
  expect(result.currentSecurityPath).toBe('#124');
});

test('G1 clean successors replace stale/overlapping historical merge paths without declaring DONE',async()=>{
  ...
  expect(byId.get('G1-SEC-MIG').canonical).toEqual(['hapo3005/hoy#124']);
  ...
  for(const old of ['hapo3005/hoy#103','hapo3005/hoy#105','hapo3005/hoy#107','hapo3005/hoy#109'])expect(active).not.toContain(old);
