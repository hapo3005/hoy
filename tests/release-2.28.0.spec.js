...
test('HOY 2.28.2 decision assets remain wired in the current release',async({page,request})=>{
  const [js,css,index,worker]=await Promise.all([
    request.get('./guest-decision-core-2.28.js'),request.get('./guest-decision-core-2.28.css'),request.get('./index.html'),request.get('./service-worker.js')
  ]);
