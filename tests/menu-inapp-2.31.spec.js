...
async function ready(page){
  ...
  await page.waitForFunction(()=>Array.isArray(DATA)&&DATA.length>0&&window.hoyMenuInAppVersion==='2.31.0');
}

test('HOY 2.31 in-app menu assets are wired and cached',async({request})=>{
...
test('Soul Kitchen renders all official menu pages inside HOY',async({page})=>{
  await ready(page);
  ...
  expect(menu.provenanceUrl).toContain('menurestauranteqr.es/soulkitchen');
...
test('source-only menu is not presented as a finished external menu',async({page})=>{
  await ready(page);
  const html=await page.evaluate(()=>{const p=DATA[0],old=MENUS[p.id];MENUS[p.id]={status:'source_only',source:null,provenanceUrl:'https://example.com/menu',label:'Offizielle Quelle'};try{return menuPanel(p)}finally{if(old)MENUS[p.id]=old;else delete MENUS[p.id]}});
