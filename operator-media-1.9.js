...
(function(){
  ...
  let reviewState={restaurantId:null,canApprove:false,claimStatus:'none',candidates:[],decisions:new Map(),replacementUploaded:new Set(),loading:false};
  ...
  function mediaPartnerPanel(p,claim){
    ...
    return `<div class="owner-media-panel"><div class="owner-media-copy"><div class="eyebrow">BILDER · HOY BEREITET VOR</div><h3>Ihre Bildauswahl ist vorbereitet.</h3><p>${verified?'Prüfen Sie unsere Vorauswahl, geben Sie alles mit einem Klick frei oder ersetzen Sie einzelne Motive.':'Sie können die vorausgewählten Quellen bereits ansehen. Die eigentliche Freigabe wird nach der HOY-Verifizierung Ihres Betreiberkontos aktiviert.'}</p></div><button class="${verified?'primary':''}" data-media-review="${p.id}">${verified?'Bilder prüfen & freigeben':'Vorauswahl ansehen'}</button></div>`;
  }
  ...
  partner=function(){
    ...
    const panel=mediaPartnerPanel(p,claim);
    return html.replace('<div class="partner-hero">',panel+'<div class="partner-hero">');
  };

  // Claim step 3 no longer asks a new partner to do work that HOY can prepare for them.
  claimStepThree=function(p){
    return `<h2>Bilder ohne Extraarbeit.</h2><p class="claim-lead">HOY bereitet für Ihr Profil bereits eine passende Bildauswahl aus Ihren offiziellen Unternehmenskanälen vor. Sie müssen jetzt nichts hochladen.</p>${claimRestaurantCard(p)}<div class="claim-card good"><h3>Nach der Verifizierung</h3><p>Sie sehen unsere Vorauswahl direkt im Partnerbereich: Titelbild, Essen, Terrasse, Drinks oder Event. Dann können Sie alles freigeben, einzelne Motive ablehnen oder mit einem eigenen Bild ersetzen.</p></div><div class="claim-card notice"><h3>Bis zur Freigabe bleibt alles privat.</h3><p>Kein vorausgewähltes Unternehmensbild wird öffentlich in HOY verwendet, bevor ein verifizierter Betreiber es bestätigt hat. Die bisherigen Regionalbilder bleiben nur der Fallback.</p></div>`;
  };
  ...
  function mediaCandidateCard(c){
    const decision=decisionFor(c);const disabled=reviewState.canApprove?'':' disabled';const direct=!!c.asset_url;
    return `<article class="media-review-card" data-candidate="${c.id}">${candidatePreview(c)}<div class="media-review-body"><div class="media-review-meta"><span>${esc(mediaSourceLabel(c.source_type))}</span>${c.is_public?'<span class="live">LIVE</span>':''}</div><p>${esc(c.curation_note||'HOY hat diese Quelle für Ihr Profil vorbereitet.')}</p><a href="${esc(c.source_page_url)}" target="_blank" rel="noopener noreferrer">Offizielle Quelle öffnen ↗</a>${reviewState.canApprove?`<div class="media-decisions"><button data-media-decision="approved" data-candidate-id="${c.id}" class="${decision==='approved'?'active':''}"${disabled}>${direct?'Freigeben':'Quelle freigeben'}</button><button data-media-decision="rejected" data-candidate-id="${c.id}" class="${decision==='rejected'?'active':''}"${disabled}>Nicht verwenden</button><button data-media-decision="replace_requested" data-candidate-id="${c.id}" class="${decision==='replace_requested'?'active':''}"${disabled}>Ersetzen</button></div>${decision==='replace_requested'?`<label class="media-replace"><input type="file" accept="image/jpeg,image/png,image/webp" data-media-replace-file="${c.id}"><b>${reviewState.replacementUploaded.has(Number(c.id))?'Ersatzbild gespeichert ✓':'Eigenes Ersatzbild auswählen'}</b><small>Max. 10 MB · bleibt bis zur Veröffentlichung privat.</small></label>`:''}`:''}</div></article>`;
  }
  function renderMediaReview(){
    ...
    d.innerHTML=`<div class="media-review-flow"><div class="claim-head"><button class="round" data-media-close>${icons.back}</button><span class="claim-step">HOY · BILDER FREIGEBEN</span></div><div class="media-review-title"><div class="eyebrow">${can?'BETREIBER VERIFIZIERT':'VORSCHAU · CLAIM IN PRÜFUNG'}</div><h2>${esc(p?.name||'Ihr Profil')}</h2><p>${can?'Wir haben die Bildwelt vorbereitet. Sie entscheiden nur noch, was HOY verwenden darf.':'Die Vorauswahl ist schon sichtbar. Nach der HOY-Verifizierung können Sie sie mit wenigen Klicks freigeben.'}</p></div>${candidates.length?`<div class="media-review-grid">${candidates.map(mediaCandidateCard).join('')}</div>`:'<div class="claim-card notice"><h3>Noch keine Medienauswahl.</h3><p>HOY recherchiert gerade die offiziellen Bildquellen dieses Betriebs.</p></div>'}${can&&candidates.length?`<label class="media-rights"><input type="checkbox" data-media-rights><span>Ich bestätige, dass mein Unternehmen die erforderlichen Rechte an den freigegebenen Bildern bzw. den von mir freigegebenen Unternehmensquellen besitzt oder HOY deren Nutzung ausdrücklich gestatten darf.</span></label><div class="media-review-actions"><button data-media-all>Alle freigeben</button><button class="primary" data-media-save>Auswahl speichern</button></div><p class="media-review-note">Direkt ausgewählte Website-Bilder werden nach Ihrer Freigabe in den HOY-Medienspeicher übernommen. Bei Social-/Verzeichnisquellen speichert HOY zunächst Ihre Quellenfreigabe und wählt anschließend das konkrete Betreiberbild aus.</p>`:''}</div>`;
  ...
  async function openMediaReview(restaurantId){
    const d=ensureMediaDialog();d.showModal();
  ...
  async function submitMediaReview(){
    ...
    if(!d.querySelector('[data-media-rights]')?.checked){toast('Bitte Nutzungsfreigabe bestätigen');return;}
    ...
    await fetchMediaReview(reviewState.restaurantId);render();
  }
  async function uploadReplacement(candidate,file){
    const d=ensureMediaDialog();
    if(!reviewState.canApprove||!cloud.user)return;
    if(!d.querySelector('[data-media-rights]')?.checked){toast('Bitte zuerst Nutzungsrechte bestätigen');return;}
