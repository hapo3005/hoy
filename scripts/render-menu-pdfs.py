...
def download_pdf(url: str, target: pathlib.Path) -> tuple[str, int]:
    request = urllib.request.Request(url, headers={
        'User-Agent': 'Mozilla/5.0 HOY-Menu-PDF-Renderer/2.28.2',
...
def main() -> int:
    ...
    manifest = {'renderer': 'HOY 2.28.2', 'targets': []}
    for target in cfg.get('targets', []):
        ...
        print(f'✓ {rid} {label}: {count} rendered pages')
