...
if(reg.schemaVersion!=='1.1.1')fail(`Unexpected G1 schema version: ${reg.schemaVersion}`);
...
const requiredIds=['G1-INTEGRATION','G1-SEC-MIG','G1-CORP-IP','G1-DIGITAL-CONTROL','G1-OSS-SBOM','G1-DATA-RIGHTS','G1-BUSINESS-TERMS','G1-PRIVACY','G1-PUBLIC-RUNTIME','G1-BRAND','G1-PLATFORM-TRANSFER','G1-UMBRELLA'];
...
if(umbrella?.status!=='REFERENCE_ONLY'||!umbrella.canonical.includes('hapo3005/hoy#102'))fail('Historical umbrella must remain PR #102 REFERENCE_ONLY');

const exactCanonical={
  'G1-INTEGRATION':['hapo3005/hoy#118'],
  'G1-SEC-MIG':['hapo3005/hoy#124'],
...
for(const ref of ['hapo3005/hoy#104','hapo3005/hoy#123'])if(!corp.includes(ref))fail(`G1-CORP-IP missing ${ref}`);
...
for(const forbidden of ['hapo3005/hoy#103','hapo3005/hoy#105','hapo3005/hoy#107','hapo3005/hoy#109']){
...
for(const phrase of ['evidence fragmentation','One canonical merge/evidence source per gate.','Archive acquired-state evidence.','No hidden exceptions.','does not itself authorize','Platform Architecture & Transferability','Core PR #119','Core PR #120','Core PR #121','Core PR #122','Core PR #123','Core PR #124','95 registered migrations','not safe to apply unchanged']){
...
console.log(JSON.stringify({ok:true,gate:reg.gate,schemaVersion:reg.schemaVersion,overallStatus:reg.overallStatus,items:reg.items.length,done:reg.items.filter(x=>x.status==='DONE').length,greenCandidate:reg.items.filter(x=>x.status==='GREEN_CANDIDATE').length,inProgress:reg.items.filter(x=>x.status==='IN_PROGRESS').length,blocked:reg.items.filter(x=>x.status==='BLOCKED').length,referenceOnly:reg.items.filter(x=>x.status==='REFERENCE_ONLY').length,supersededCanonicalPathsBlocked:true,currentSecurityPath:'#124'},null,2));
