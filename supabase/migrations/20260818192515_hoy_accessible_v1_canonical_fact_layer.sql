-- Applied to HOY La Manga production as migration 20260818192515.
-- Canonical HOY Accessible fact layer with fail-closed public reads.
...
create unique index if not exists restaurant_accessibility_one_current_fact on public.restaurant_accessibility_facts (restaurant_id, feature_key) where is_current;
create index if not exists restaurant_accessibility_facts_restaurant_idx on public.restaurant_accessibility_facts (restaurant_id) where is_current;
create index if not exists restaurant_accessibility_facts_feature_idx on public.restaurant_accessibility_facts (feature_key, status) where is_current;
...
create policy "anon reads active accessibility features" on public.accessibility_feature_registry for select to anon using (is_active = true);
create policy "authenticated reads active or admin accessibility features" on public.accessibility_feature_registry for select to authenticated using (is_active = true or private.is_hoy_admin());
create policy "hoy admins insert accessibility features" on public.accessibility_feature_registry for insert to authenticated with check (private.is_hoy_admin());
create policy "hoy admins update accessibility features" on public.accessibility_feature_registry for update to authenticated using (private.is_hoy_admin()) with check (private.is_hoy_admin());
create policy "hoy admins delete accessibility features" on public.accessibility_feature_registry for delete to authenticated using (private.is_hoy_admin());
...
create policy "anon reads current published accessibility facts" on public.restaurant_accessibility_facts for select to anon using (
  is_current = true
  and exists (select 1 from public.restaurants r where r.id=restaurant_accessibility_facts.restaurant_id and r.is_published=true)
  and (verification_level in ('hoy_verified','business_confirmed','community_confirmed') or status='unknown' or nullif(btrim(source_url),'') is not null)
);
create policy "authenticated reads published or admin accessibility facts" on public.restaurant_accessibility_facts for select to authenticated using (
...
create policy "hoy admins insert accessibility facts" on public.restaurant_accessibility_facts for insert to authenticated with check (private.is_hoy_admin());
create policy "hoy admins update accessibility facts" on public.restaurant_accessibility_facts for update to authenticated using (private.is_hoy_admin()) with check (private.is_hoy_admin());
create policy "hoy admins delete accessibility facts" on public.restaurant_accessibility_facts for delete to authenticated using (private.is_hoy_admin());
...
on conflict (feature_key) do update set
 label_de=excluded.label_de,scope=excluded.scope,priority=excluded.priority,data_type=excluded.data_type,unit=excluded.unit,
 freshness_external_days=excluded.freshness_external_days,freshness_verified_days=excluded.freshness_verified_days,
 matching_mode=excluded.matching_mode,notes=excluded.notes,is_active=true,updated_at=now();

insert into public.restaurant_accessibility_facts
(fact_ref,restaurant_id,feature_key,status,source_type,verification_level,evidence_type,source_url,checked_at,stale_after,review_state,legacy_class,secondary_hint,legacy_restrictions,evidence,is_current)
select
 'gastro-legacy-'||a.restaurant_id::text||'-'||replace(v.feature_key,'.','-'), a.restaurant_id,v.feature_key,v.status,
 ...
 case when a.verification_source='public_research' and v.status<>'unknown' and nullif(btrim(a.source_url),'') is null then 'review_needed' else 'clean' end,
 a.overall_status,a.secondary_note,a.accessibility_note,
...
insert into public.restaurant_accessibility_facts
(fact_ref,restaurant_id,feature_key,status,source_type,verification_level,evidence_type,source_url,checked_at,stale_after,review_state,legacy_class,legacy_restrictions,evidence,is_current)
select 'gastro-legacy-'||a.restaurant_id::text||'-access-hearing_loop',a.restaurant_id,'access.hearing_loop','unknown',
...
from public.restaurant_accessibility a where a.restaurant_id in (22,112,145,174)
on conflict (fact_ref) do nothing;

do $$ begin
 if not exists (select 1 from pg_constraint where conname='accessibility_external_claim_requires_source_url' and conrelid='public.restaurant_accessibility_facts'::regclass) then
  alter table public.restaurant_accessibility_facts add constraint accessibility_external_claim_requires_source_url check (
   verification_level <> 'external_unverified' or status='unknown' or nullif(btrim(source_url),'') is not null
  ) not valid;
 end if;
end $$;
...
declare v_verification text; v_source_type text; v_checked_at timestamptz; v_verified_at timestamptz; v_revision text;
begin
 v_verification := case new.verification_source when 'operator' then 'business_confirmed' when 'onsite' then 'hoy_verified' else 'external_unverified' end;
 v_source_type := case new.verification_source when 'operator' then 'operator_confirmation' when 'onsite' then 'hoy_onsite_verification' else 'public_structured_business_data' end;
 ...
 update public.restaurant_accessibility_facts set is_current=false,updated_at=now()
 where restaurant_id=new.restaurant_id and feature_key in ('access.step_free','access.wheelchair_seating','access.toilet','access.parking','access.hearing_loop') and is_current=true;
 insert into public.restaurant_accessibility_facts
 (fact_ref,restaurant_id,feature_key,status,source_type,verification_level,evidence_type,source_url,checked_at,verified_at,stale_after,review_state,legacy_class,secondary_hint,legacy_restrictions,evidence,is_current)
 select 'gastro-sync-'||new.restaurant_id::text||'-'||replace(v.feature_key,'.','-')||'-'||v_revision,
  new.restaurant_id,v.feature_key,v.status,v_source_type,v_verification,new.evidence_type,new.source_url,v_checked_at,v_verified_at,
  v_checked_at + make_interval(days=>case when v_verification in ('hoy_verified','business_confirmed','community_confirmed') then registry.freshness_verified_days else registry.freshness_external_days end),
  case when v_verification='external_unverified' and v.status<>'unknown' and nullif(btrim(new.source_url),'') is null then 'review_needed' else 'clean' end,
  new.overall_status,new.secondary_note,new.accessibility_note,
  jsonb_build_object('sync_source','restaurant_accessibility','verification_source',new.verification_source,'synced_at',now()),true
 from (values
...
create trigger hoy_accessibility_fact_sync after insert or update of wheelchair_entrance_state,wheelchair_seating_state,wheelchair_toilet_state,accessible_parking_state,hearing_loop_state,verification_source,source_url,evidence_type,checked_at,accessibility_note on public.restaurant_accessibility for each row execute function public.hoy_sync_accessibility_facts_from_legacy();
...
comment on table public.restaurant_accessibility_facts is 'Per-feature accessibility facts. External positive/negative facts require provenance; unknown is never interpreted as no; older rows are retained for audit/history.';
comment on function public.hoy_sync_accessibility_facts_from_legacy() is 'Synchronizes legacy/operator accessibility confirmations into versioned per-feature HOY Accessible facts with DD provenance gating.';
