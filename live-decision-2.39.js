...
(function(){
  ...
  function readPlan(){
    try{return [...new Set((JSON.parse(localStorage.getItem(PLAN_KEY)||'[]')||[]).map(Number).filter(Number.isFinite))].slice(0,PLAN_LIMIT)}catch{return []}
  }
  function savePlan(ids){localStorage.setItem(PLAN_KEY,JSON.stringify([...new Set(ids.map(Number).filter(Number.isFinite))].slice(0,PLAN_LIMIT)))}
  function validPlan(){
    const ids=readPlan();
    if(!(DATA||[]).length)return ids;
    const valid=ids.filter(id=>!!restaurantById(id));
    if(valid.length!==ids.length)savePlan(valid);
    return valid;
  }
  function planHas(id){return validPlan().includes(Number(id))}
  function addPlan(id){
    const numeric=Number(id),ids=validPlan();
    if(!Number.isFinite(numeric)||!restaurantById(numeric))return false;
    if(ids.includes(numeric))return true;
    if(ids.length>=PLAN_LIMIT){if(typeof toast==='function')toast(`Dein HOY Plan ist auf ${PLAN_LIMIT} Orte begrenzt`);return false}
    savePlan([...ids,numeric]);
    if(typeof trackEvent==='function')trackEvent('live_plan_add',numeric,{surface:'live239'});
    render();return true;
  }
  function removePlan(id){
    const numeric=Number(id),ids=validPlan();if(!ids.includes(numeric))return false;
    savePlan(ids.filter(x=>x!==numeric));
    if(typeof trackEvent==='function')trackEvent('live_plan_remove',numeric,{surface:'live239'});
    render();return true;
  }
  function clearPlan(){
    if(!validPlan().length)return false;
    savePlan([]);if(typeof trackEvent==='function')trackEvent('live_plan_clear',null,{surface:'live239'});render();return true;
  }
  ...
  function distanceLabel(km){if(km==null||!Number.isFinite(km))return '';if(km<1)return `${Math.max(50,Math.round(km*1000/50)*50)} m`;return `${km.toFixed(km<10?1:0).replace('.',',')} km`}
  ...
  function cardFor(p,{nearby=false,now=new Date()}={}){
    const km=distanceFor(p),inPlan=planHas(p.id),full=!inPlan&&validPlan().length>=PLAN_LIMIT;
    return `<article class="live239-card" data-live239-card="${Number(p.id)}">
      ...
      <button type="button" class="live239-plan-toggle ${inPlan?'active':''}" data-live239-plan-toggle="${Number(p.id)}" aria-pressed="${inPlan?'true':'false'}" ${full?'disabled aria-disabled="true"':''}>${inPlan?'Im Plan ✓':full?'Plan voll':'Zum Plan +'}</button>
    </article>`;
  }
  ...
  function nearbyRows(now=new Date()){
    const all=(DATA||[]).filter(p=>coordsFor(p)).map(p=>({p,km:distanceFor(p),state:nowStatus(p,now)?.state||'unknown'})).filter(x=>x.km!=null).sort((a,b)=>a.km-b.km);
    const usable=all.filter(x=>x.state==='open'||x.state==='later');
    const chosen=[];
    for(const row of usable){if(chosen.length>=3)break;chosen.push(row)}
    for(const row of all){if(chosen.length>=3)break;if(!chosen.includes(row))chosen.push(row)}
  ...
  function planMarkup(now=new Date()){
    const rows=validPlan().map(restaurantById).filter(Boolean);
  ...
  function requestLocation(){
    if(live.geoState==='requesting')return;
    ...
    navigator.geolocation.getCurrentPosition(pos=>{
      ...
      if(!Number.isFinite(lat)||lat<-90||lat>90||!Number.isFinite(lon)||lon<-180||lon>180){live.geoState='error';live.geoError='Der Standort war ungültig.';render();return}
      live.position={lat,lon};live.geoState='ready';
      if(typeof trackEvent==='function')trackEvent('live_nearby_enabled',null,{surface:'live239'});render();
    },err=>{
      live.geoState='error';live.geoError=err?.code===1?'Standortfreigabe wurde nicht erteilt.':err?.code===3?'Standortbestimmung hat zu lange gedauert.':'Standort konnte gerade nicht bestimmt werden.';render();
  ...
  wire=function(){
    ...
    document.querySelectorAll('[data-live239-plan-toggle]').forEach(b=>b.onclick=e=>{e.stopPropagation();if(b.disabled)return;const id=Number(b.dataset.live239PlanToggle);planHas(id)?removePlan(id):addPlan(id)});
  ...
  window.hoyLiveDecision239={readPlan:validPlan,addPlan,removePlan,requestLocation,distanceFor,timelineRows,nearbyRows};
})();
