...
## Active technical candidates

### PR #112 — RT-006 supply-chain / reproducibility
Reopened against current `main` on 19.08.2026.

Candidate contains:
- package-lock v3
- `npm ci --ignore-scripts` for QA dependency installs
- immutable SHA pins for external GitHub Actions
- read-only fail-closed RT-006 verification workflow
- supply-chain evidence document

Merge rule:
- keep Draft until fresh RT-006 + Final Release + Critical + Browser QA are all successful on current-main merge context
- inspect final diff before merge
- no production/Supabase deploy is implied by merge

### PR #109 — privacy analytics fail-closed hotfix
Reopened against current `main` on 19.08.2026.

Candidate purpose:
- no persistent analytics id/session/history/pilot attribution before explicit granted consent
...
Merge rule:
- fresh Final Release + Critical + Browser QA must pass
- patch must remain limited to consent-gating/regression coverage

## Technical items still open after those candidates
...
4. **Release SBOM / notices / AI asset register**
   - fresh release-candidate SBOM after supply-chain merge
   - final licence/unknown/copyleft review
   - Third-Party Notices
   - AI Asset Register
...
Finish #112 and #109 on fresh QA, then create the release-candidate SBOM/notices/AI-register pack and the controlled auth-test harness. After that, remaining G1 blockers are predominantly external/legal/ownership rather than additional product feature work.
