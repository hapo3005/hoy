/* HOY Control 2.28.0 — authenticated official-site menu auto-discovery */
(function(){
  if(window.__hoyAdminMenuAutoDiscovery228)return;
  window.__hoyAdminMenuAutoDiscovery228=true;
  window.hoyAdminMenuAutoDiscoveryVersion='2.28.0';

  const CORE_AREAS=new Set(['La Manga del Mar Menor','Cabo de Palos']);
  const FOOD_TYPES=new Set(['restaurant','chiringuito','beach_club']);
  ...
  function missingRows228(area=null,foodOnly=false){return coreRows228(area).filter(r=>!hasCore228(r.id)&&(!foodOnly||FOOD_TYPES.has(clean228(r.venue_type))))}
  function runnable228(area=null,foodOnly=false){return missingRows228(area,foodOnly).filter(ownedWebsite228)}
  function metrics228(){
    const all=coreRows228(),food=all.filter(r=>FOOD_TYPES.has(clean228(r.venue_type))),foodReady=food.filter(r=>hasCore228(r.id)),missing=all.filter(r=>!hasCore228(r.id));
