/* HOY Control 2.29.1 — official source -> internal draft -> explicit review -> atomic publish */
(function(){
  if(window.__hoyAdminMenuEditorial229)return;
  window.__hoyAdminMenuEditorial229=true;
  window.hoyAdminMenuEditorialVersion='2.29.1';
  ...
  function wireDialog229(d,j){
    ...
    d.querySelector('[data-mei229-confirm-all]')?.addEventListener('click',async()=>{if(!confirm('Hast du wirklich alle extrahierten Positionen gegen die offizielle Quelle geprüft?'))return;try{await edge229('confirm_all',{import_id:j.id});await open229(j.id,false)}catch(e){toast(e?.message||'Bestätigung fehlgeschlagen')}});
    ...
    d.querySelector('[data-mei229-publish]')?.addEventListener('click',async()=>{if(!confirm('Diese vollständig geprüfte Karte jetzt als HOY-Hauptkarte veröffentlichen?'))return;try{const data=await edge229('publish',{import_id:j.id});toast(`${data.published_items||0} geprüfte Positionen veröffentlicht`);await loadData();d.close();render()}catch(e){toast(e?.message||'Veröffentlichung fehlgeschlagen')}});
  }
