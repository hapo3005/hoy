/* HOY 2.17.1 — transparent sponsored event placement; eligibility callback fixed and observable */
(function(){
  ...
  function promoForOffer(id){return (promoState.byOffer.get(String(id))||[]).find(row=>guestEligible(row))||null}
  ...
  async function loadPublicPromotions(){
    ...
    try{
      ...
      // Never pass guestEligible directly to Array.filter: filter also passes the numeric index as arg #2.
      promoState.rows=(data||[]).map(normalizePromo).filter(row=>guestEligible(row));
      promoState.byOffer=new Map();
      for(const row of promoState.rows){
        const key=String(row.offer_id),list=promoState.byOffer.get(key)||[];list.push(row);promoState.byOffer.set(key,list);
      }
      promoState.loaded=true;promoState.error='';
    }catch(err){
      promoState.rows=[];promoState.byOffer=new Map();promoState.loaded=true;promoState.error=err?.message||String(err);
      console.warn('HOY promotions unavailable',err);
    }
  }
  window.hoyLoadPublicPromotions=loadPublicPromotions;
  window.hoyPromotionSummary=()=>({loaded:promoState.loaded,eligible:promoState.rows.length,error:promoState.error});
  ...
  function pickHighlight(){
    const candidates=promoState.rows.map(promoTuple).filter(Boolean);if(!candidates.length)return null;
  ...
  home=function(){
    const html=baseHome217(),tuple=pickHighlight();if(!tuple)return html;
    ...
    // Duplicate suppression only: the sponsored event receives a separate labeled slot; organic venue order is untouched.
  ...
  listCard=function(p){
    const html=baseListCard217(p),active=venuePromotion(p);if(!active)return html;
  ...
  mapView=function(){
    ...
    root.querySelectorAll('.map-decision-card[data-map-card]').forEach(card=>{
      const p=(DATA||[]).find(x=>Number(x.id)===Number(card.dataset.mapCard)),active=venuePromotion(p);if(!active)return;
      card.classList.add('has-sponsored-event');
  ...
  function decorateProfilePromotions(id){
    ...
    items.forEach((item,i)=>{
      const row=rows[i],promo=promoForOffer(row?.id);if(!promo||item.querySelector('.hoy-sponsored-chip'))return;
      item.classList.add('sponsored');item.querySelector('.hoy-current-item-top')?.insertAdjacentHTML('afterbegin',sponsoredChip());
    });
  }
  ...
  function decorateManager(){
    ...
    d.querySelectorAll('.hoy-manager-row').forEach(row=>{
      ...
      const ds=control?.dataset||{},offerId=ds.currentArchive||ds.currentEdit||ds.currentPublish||ds.currentNextWeek;if(!offerId)return;
      ...
      const promo=latestPromotion(offerId),actions=row.querySelector('.hoy-manager-actions');if(!actions)return;
  ...
  function showRequestPanel(offerId){
    const d=document.getElementById('hoyCurrentManager'),offer=operatorState.offers.find(x=>String(x.id)===String(offerId));if(!d||!offer)return;
  ...
  async function requestPromotion(offerId){
    ...
    const start=new Date(Math.max(Date.now(),new Date(offer.starts_at).getTime()-86400000)).toISOString(),end=new Date(offer.ends_at).toISOString();
    try{
  ...
  async function syncManager(){
    const d=document.getElementById('hoyCurrentManager');if(!d?.open){operatorState.sig='';return}
