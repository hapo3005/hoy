...
(function(){
  ...
  const isEmbedded225=s=>{
    const p=s?.display_payload&&typeof s.display_payload==='object'?s.display_payload:{};
    return s?.is_official!==false&&clean225(s.completeness_status)==='complete'&&clean225(p.mode)==='official_embed'&&!!safeHttps225(p.embed_url);
  };
  ...
  function blockerRows225(){
    ...
    for(const [id,sources] of byRestaurant){
      ...
      const embedded=sources.some(s=>s.__hoyEmbedded225||isEmbedded225(s));
      const structured=sources.some(s=>['complete','image_complete'].includes(clean225(s.completeness_status))&&!s.__hoyEmbedded225);
      const sourceOnly=sources.filter(s=>clean225(s.completeness_status)==='source_only');
      if(!embedded&&!structured&&sourceOnly.length){
        const primary=[...sourceOnly].sort((a,b)=>(clean225(b.coverage_scope)==='full_menu')-(clean225(a.coverage_scope)==='full_menu'))[0];
        out.push({r,source:primary,all:sourceOnly});
      }
    }
    return out.sort((a,b)=>{
